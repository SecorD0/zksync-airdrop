...
class General:
    ...
    def import_addresses() -> None:
        try:
            ...
            for row in read_spreadsheet(path=config.ADDRESSES_FILE, sheet_name='Addresses'):
                ...
                if address:
                    db.insert(Address(address=address.lower()))
                    imported += 1
    ...
    def export_addresses() -> None:
        try:
            addresses = list(db.execute('SELECT * FROM addresses'))
            if addresses:
                ...
                for column, header in enumerate(['n'] + list(db.execute('SELECT * FROM addresses').keys())[1:]):
